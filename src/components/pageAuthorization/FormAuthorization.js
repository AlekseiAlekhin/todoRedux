...
export const FormAuthorization = memo(()=>{
    ...
    let name = useRef();
    let pass = useRef();
    return <div style={{padding: '15px 20px', boxSizing: 'border-box'}}>
        ...
        <BDiv mx="auto" shadow="lg" mb="5" bg="light" rounded style={{maxWidth: '400px',padding: '15px 20px', boxSizing: 'border-box'}}>
            <h2>LogIn</h2>
            <BDiv style={{display: 'flex', justifyContent: 'space-between', alignItems: 'center'}}><p>Login</p><input ref={name} type={"input"} name={'name'} required style={{marginBottom:'5px'}}/></BDiv>
            <BDiv style={{display: 'flex', justifyContent: 'space-between', alignItems: 'center'}}><p>Password</p><input ref={pass} type="password" name={'password'} required style={{marginBottom:'5px'}}/></BDiv>
